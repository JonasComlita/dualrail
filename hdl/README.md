...
- OSS CAD Suite via Scoop: `oss-cad-suite-nightly`
- Verible from the CHIPS Alliance Windows release, exposed through Scoop shims

Run all current HDL simulation tests:

```powershell
./hdl/scripts/run_icarus.ps1
```

Run Verible syntax/lint checks plus Verilator lint:

```powershell
./hdl/scripts/check_verible.ps1
```
