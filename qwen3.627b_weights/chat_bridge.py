"""
chat_bridge.py — Python front-end for run_qwen.exe
...
Protocol (stdout from run_qwen.exe):
...
WEIGHTS_DIR    = os.path.join(BASE_DIR, "converted_qwen")
CONFIG_PATH    = os.path.join(BASE_DIR, "model", "config.json")

# Build output is one level up from qwen3.627b_weights/
BUILD_DIR      = os.path.join(BASE_DIR, "..", "build")
EXE_NAME       = "run_qwen.exe" if sys.platform == "win32" else "run_qwen"
RUN_QWEN_EXE = os.path.join(BUILD_DIR, EXE_NAME)
...
def run_generation(tokenizer, history: list[int], bos_id: int,
    ...
    cmd = [
        ...
        "--kv_heads",     str(config.get("num_key_value_heads", 4)),
        "--layers",       str(config.get("num_hidden_layers", 64)),
    ...
    try:
        ...
        print(red(f"\n[Error] run_qwen not found at: {RUN_QWEN_EXE}"))
...
def main():
    ...
    if not os.path.exists(RUN_QWEN_EXE):
        print(red(f"[Error] Executable not found: {RUN_QWEN_EXE}"))
    ...
    print(f"\n{bold('=== Qwen Chat ===')}  (type {dim('exit')} to quit)\n")
