...
fn main() -> t40 {
    ...
    if kload(SYS_DETAIL_ADDR) - ERR_EXISTS != 0 { return -31; }
    if vfs_lookup(0, new_path) - inode_before != 0 { return -32; }
    if kernel_syscall_dispatch(1, 59, src_dir, child_path, 0, 0) + 1 != 0 { return -33; }
    if kload(SYS_DETAIL_ADDR) - ERR_IS_DIR != 0 { return -34; }
    seed_root(root_path);
    if kernel_syscall_dispatch(1, 59, root_path, child_path, 0, 0) + 1 != 0 { return -35; }
    if kload(SYS_DETAIL_ADDR) - ERR_IS_DIR != 0 { return -36; }
    if kernel_syscall_dispatch(1, 59, -1, new_path, 0, 0) + 1 != 0 { return -37; }
    if kload(SYS_DETAIL_ADDR) - ERR_BAD_PTR != 0 { return -38; }
    if vfs_lookup(0, new_path) - inode_before != 0 { return -39; }
    if kernel_syscall_dispatch(1, 59, new_path, child_path, 0, 0) + 1 != 0 { return -40; }
    if kload(SYS_DETAIL_ADDR) - ERR_NOT_DIR != 0 { return -41; }
    ...
    if kernel_syscall_dispatch(1, 59, new_path, retry_path, 0, 0) + 1 != 0 { return -42; }
    if kload(SYS_DETAIL_ADDR) - ERR_NO_SPACE != 0 { return -43; }
    if kload(WAL_NEXT_TXID_ADDR) - before_txid != 0 { return -44; }
    if kload(dirent_addr(source_slot) + DIRENT_PARENT) - before_parent != 0 { return -45; }
    if vfs_lookup(0, new_path) - inode_before != 0 { return -46; }
    return 1;
}
)TRIT";
...
void testRenameRecovery() {
    std::cout << "[2] Atomic VFS rename WAL recovery\n";
    const std::string kernel = readTextFile("kernel.trit");
...
void testRenameCompilerSurface() {
    std::cout << "[3] Atomic VFS rename compiler and SDK surface\n";
    using namespace sandbox::compiler;
