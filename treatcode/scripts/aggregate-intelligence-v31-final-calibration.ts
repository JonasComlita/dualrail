...
const observationRoot = path.join(process.env.TREATCODE_V31_EVIDENCE_ROOT || path.join(privateRoot, "final-calibration"), "observations");
...
const calibrationPlan = JSON.parse(readFileSync(path.join(repositoryRoot, "benchmarks", "intelligence-v3.1", "calibration-plan.v3.1.json"), "utf8"));
...
const tasks = authored.sort((a, b) => a.task_id.localeCompare(b.task_id)).map((authoredTask) => {
  ...
  const selected = Object.fromEntries((["weak", "medium", "frontier"] as const).map((cohort) => [cohort, selectIntelligenceV31CalibrationFamilies(authoredTask.task_id, authoredTask.author_model_family, calibrationPlan.cohorts[cohort].assignments)]));
  return {
    ...authoredTask,
    calibration_model_families: selected.weak.map((assignment) => assignment.model_family) as [string, string],
    calibration_configurations: Object.fromEntries((["weak", "medium", "frontier"] as const).map((cohort) => [cohort, selected[cohort].map((assignment) => `${assignment.model_family}-${assignment.reasoning_effort}`)])) as Record<"weak" | "medium" | "frontier", [string, string]>,
...
contract.readiness.calibrated = output.accepted_tasks;
contract.execution_identity_readiness.provider_signed_calibration_observations = observations.filter((observation) => observation.model_execution_verified === true && /^[a-f0-9]{64}$/.test(observation.execution_attestation_sha256 || "")).length;
if (output.status === "ready_to_freeze") contract.status = "calibrated_awaiting_infrastructure_and_freeze";
