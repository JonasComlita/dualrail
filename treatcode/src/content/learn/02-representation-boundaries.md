---
{
  ...
  "level": "beginner",
  "order": 2,
  "summary": "Build the mental model for Trit representation, gates, and hardware realization.",
  ...
  "interactive": {
    ...
    "prompt": "Which value is the numeric source of truth for scalar arithmetic?",
    "options": [
      "Two bits per lane",
      "The positional numeric value",
      "A host pointer"
    ],
    "answer": 1,
    "explanation": "The repository boundary is defined by the current source and validation evidence for Trit representation, gates, and hardware realization. Planned and unavailable work remains labelled."
  }
}
---
...
No earlier Learn page is required. The Stack Explorer phase still records repository dependencies, so use the source and phase links if a term is unfamiliar. This is the mental-model lesson for Trit representation, gates, and hardware realization. Begin with the general systems idea, then compare it with the current Trit boundary before you touch a tool. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
...
[Continue to the next lesson](/learn/eecs/hardware-gates). The next page should make the dependency explicit and keep the same distinction between general concept, current Trit behavior, and planned work. When you reach the terminal lesson, write the closure report and list residual gaps instead of assuming that navigation itself proves completion.
