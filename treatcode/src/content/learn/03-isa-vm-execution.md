---
{
  ...
  "level": "beginner",
  "order": 4,
  "summary": "Build the mental model for ISA, registers, privilege, CSRs, traps, and ABI.",
  ...
  "next": "isa-trap-abi-trace",
  "interactive": {
    "kind": "choice",
    "title": "Decode check",
    "prompt": "Which statement keeps the isa-binary boundary honest?",
    "options": [
    ...
    "answer": 1,
    "explanation": "The repository boundary is defined by the current source and validation evidence for ISA, registers, privilege, CSRs, traps, and ABI. Planned and unavailable work remains labelled."
  }
}
---
...
Read [hardware-gates](/learn/eecs/hardware-gates) first. Those lessons introduce the state and vocabulary that this page assumes. This is the mental-model lesson for ISA, registers, privilege, CSRs, traps, and ABI. Begin with the general systems idea, then compare it with the current Trit boundary before you touch a tool. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
...
[Continue to the next lesson](/learn/eecs/isa-trap-abi-trace). The next page should make the dependency explicit and keep the same distinction between general concept, current Trit behavior, and planned work. When you reach the terminal lesson, write the closure report and list residual gaps instead of assuming that navigation itself proves completion.
