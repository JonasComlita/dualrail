---
{
  ...
  "id": "vm-memory-trace",
  "title": "Trace a memory transition",
  "module": "Phase 04 · VM architectural state and memory system",
  "level": "beginner",
  "order": 9,
  "summary": "Trace the repository evidence for VM architectural state and memory system.",
  ...
  "interactive": {
    ...
    "exercise_id": "vm-memory-trace",
    "runner": "treatcode-learning-compiler",
    "explanation": "Submit the bounded TCL example to the repository-backed learning runner. It may pass or report a real compiler failure; either result is evidence about the VM architectural state and memory system exercise, not a local text-only success."
  }
}
---
...
Read [vm-state-memory](/learn/eecs/vm-state-memory) first. Those lessons introduce the state and vocabulary that this page assumes. This is the build, trace, and evidence lesson for VM architectural state and memory system. The goal is to inspect a real repository transition and report both what the current tests prove and what they do not prove. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
...
Read the VM state struct and trace a multiwidth test from initialization through fetch. Record the state fields that are compared after execution. Then introduce an invalid address in a scratch thought experiment and identify which invariant should fail first. The same reasoning scales beyond the example. Name the input, the state that changes, the owner of that state, the output, and the test that would catch a regression. If one of those is missing, say so. A source link can help a reader continue, but it cannot substitute for the prose that explains why the source matters.
