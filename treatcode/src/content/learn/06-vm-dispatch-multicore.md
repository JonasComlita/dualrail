---
{
  ...
  "id": "vm-dispatch-multicore",
  "title": "Interpreter, dispatch, and multicore state",
  "module": "Phase 05 · VM interpreter, dispatch, trace JIT, and multicore execution",
  "level": "beginner",
  "order": 10,
  "summary": "Build the mental model for VM interpreter, dispatch, trace JIT, and multicore execution.",
  ...
  "next": "vm-jit-benchmark",
  "interactive": {
    "kind": "trace",
    "title": "VM interpreter, dispatch, trace JIT, and multicore execution trace",
    "prompt": "Advance through the vm-execution transition and inspect the state at each boundary.",
    "steps": [
      {
        "label": "Input contract",
        "state": "a valid VM state and an execution backend choice",
        "explanation": "Start with a valid VM state and an execution backend choice."
      },
      {
        "label": "State or artifact check",
        "state": "For a three-instruction loop, the interpreter fetches and decodes every iteration",
        "explanation": "Compare the VM's dispatch entry point with the decoded trace document and the execution-backend benchmark."
      },
      {
        "label": "Boundary transition",
        "state": "a deterministic or explicitly scheduled sequence of state transitions with measured tradeoffs",
        "explanation": "The transition leaves the vm-execution boundary only when its invariant holds."
      },
      {
...
Read [vm-memory-trace](/learn/eecs/vm-memory-trace) first. Those lessons introduce the state and vocabulary that this page assumes. This is the mental-model lesson for VM interpreter, dispatch, trace JIT, and multicore execution. Begin with the general systems idea, then compare it with the current Trit boundary before you touch a tool. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
...
[Continue to the next lesson](/learn/eecs/vm-jit-benchmark). The next page should make the dependency explicit and keep the same distinction between general concept, current Trit behavior, and planned work. When you reach the terminal lesson, write the closure report and list residual gaps instead of assuming that navigation itself proves completion.
