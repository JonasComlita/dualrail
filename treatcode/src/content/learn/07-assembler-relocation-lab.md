---
{
  ...
  "id": "assembler-relocation-lab",
  "title": "Inspect encoding and relocation",
  "module": "Phase 06 · Assembler, disassembler, relocations, and binary contract",
  "level": "beginner",
  "order": 13,
  "summary": "Trace the repository evidence for Assembler, disassembler, relocations, and binary contract.",
  ...
  "next": "low-level-ir-linking",
  "interactive": {
    "kind": "source",
    "title": "Source investigation",
    "prompt": "Find the exact implementation boundary for Assembler, disassembler, relocations, and binary contract.",
    "sourcePath": "ternary_asm.h",
    "sourceHref": "https://github.com/JonasComlita/dualrail/blob/02f764a96b6a9ce98b3b63a07a0771b04d1a627d/ternary_asm.h",
    "expectedIncludes": [
      "ternary_asm.h"
    ],
    "explanation": "Open the source and compare it with the tc:test:test-tcl-asm validation record. This exercise reports the repository boundary instead of pretending that a link is a lesson."
  }
}
---
...
Read [assembler-binary-contract](/learn/eecs/assembler-binary-contract) first. Those lessons introduce the state and vocabulary that this page assumes. This is the build, trace, and evidence lesson for Assembler, disassembler, relocations, and binary contract. The goal is to inspect a real repository transition and report both what the current tests prove and what they do not prove. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
...
Take one short TASM fixture and follow it through parse, encode, and test expectation. Locate the relocation or symbol record in the source headers. Then inspect a golden test that fails on an incompatible encoding; classify the failure as syntax, encoding, relocation, or contract drift. The same reasoning scales beyond the example. Name the input, the state that changes, the owner of that state, the output, and the test that would catch a regression. If one of those is missing, say so. A source link can help a reader continue, but it cannot substitute for the prose that explains why the source matters.
...
[Continue to the next lesson](/learn/eecs/low-level-ir-linking). The next page should make the dependency explicit and keep the same distinction between general concept, current Trit behavior, and planned work. When you reach the terminal lesson, write the closure report and list residual gaps instead of assuming that navigation itself proves completion.
