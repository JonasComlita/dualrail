---
{
  ...
  "title": "Bootstrap and self-hosting",
  "module": "Phase 10 · Self-hosted TCL compiler",
  "level": "programmer",
  "order": 20,
  "summary": "Build the mental model for Self-hosted TCL compiler.",
  ...
  "next": "self-hosted-reproducibility",
  "interactive": {
    "kind": "trace",
    "title": "Self-hosted TCL compiler trace",
    "prompt": "Advance through the self-hosted-compiler transition and inspect the state at each boundary.",
    "steps": [
      {
        "label": "Input contract",
        "state": "a host compiler, TCL compiler sources, and a bootstrap plan",
        "explanation": "Start with a host compiler, TCL compiler sources, and a bootstrap plan."
      },
      {
        "label": "State or artifact check",
        "state": "Stage A uses the host compiler to build a small TCL compiler",
        "explanation": "Read the frontend and backend entry points and locate the tests that exercise native ulib or compiler pipeline behavior."
      },
      {
        "label": "Boundary transition",
        "state": "a staged compiler claim with independent checks for each trust boundary",
        "explanation": "The transition leaves the self-hosted-compiler boundary only when its invariant holds."
      },
      {
        "label": "Observable result",
        "state": "Evidence: tc:test:next-compiler-native-ulib",
        "explanation": "Use tc:test:next-compiler-native-ulib to validate the observed result."
      }
    ]
  }
}
...
Read [compiler-object-trace](/learn/eecs/compiler-object-trace) first. Those lessons introduce the state and vocabulary that this page assumes. This is the mental-model lesson for Self-hosted TCL compiler. Begin with the general systems idea, then compare it with the current Trit boundary before you touch a tool. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
