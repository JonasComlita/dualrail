---
{
  ...
  "level": "programmer",
  "order": 24,
  "summary": "Build the mental model for Reset, boot, trap entry, and first executable programs.",
  ...
  "next": "first-executable-boot",
  "interactive": {
    "kind": "trace",
    "title": "Reset, boot, trap entry, and first executable programs trace",
    "prompt": "Advance through the boot-trap transition and inspect the state at each boundary.",
    "steps": [
      {
        "label": "Input contract",
        "state": "reset state and a bootable image",
        "explanation": "Start with reset state and a bootable image."
      },
      {
        "label": "State or artifact check",
        "state": "At reset, the PC and privilege state are known",
        "explanation": "Read the bootloader and trap stub together, then open the bring-up and OS tests."
      },
      {
        "label": "Boundary transition",
        "state": "a first executable with a diagnosable path from reset to service call",
        "explanation": "The transition leaves the boot-trap boundary only when its invariant holds."
      },
      {
        "label": "Observable result",
        "state": "Evidence: tc:test:test-architecture-v2",
        "explanation": "Use tc:test:test-architecture-v2 to validate the observed result."
      }
    ]
  }
}
...
Read [runtime-streams-errors](/learn/eecs/runtime-streams-errors) first. Those lessons introduce the state and vocabulary that this page assumes. This is the mental-model lesson for Reset, boot, trap entry, and first executable programs. Begin with the general systems idea, then compare it with the current Trit boundary before you touch a tool. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
...
[Continue to the next lesson](/learn/eecs/first-executable-boot). The next page should make the dependency explicit and keep the same distinction between general concept, current Trit behavior, and planned work. When you reach the terminal lesson, write the closure report and list residual gaps instead of assuming that navigation itself proves completion.
