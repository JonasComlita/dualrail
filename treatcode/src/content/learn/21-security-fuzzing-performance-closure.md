---
{
  ...
  "id": "security-fuzzing-performance-closure",
  "title": "Security, fuzzing, and performance closure",
  "module": "Phase 20 · Security, fuzzing, performance, and full-system closure",
  "level": "eecs",
  "order": 40,
  "summary": "Build the mental model for Security, fuzzing, performance, and full-system closure.",
  ...
  "interactive": {
    "kind": "choice",
    "title": "Concept check",
    "prompt": "What must come before a performance claim?",
    "options": [
      "A correctness oracle and workload profile",
      "A larger title",
      "An unavailable asset"
    ],
    "answer": 1,
    "explanation": "The repository boundary is defined by the current source and validation evidence for Security, fuzzing, performance, and full-system closure. Planned and unavailable work remains labelled."
  }
}
---
...
Read [developer-extension-workflow](/learn/eecs/developer-extension-workflow) first. Those lessons introduce the state and vocabulary that this page assumes. This is the mental-model lesson for Security, fuzzing, performance, and full-system closure. Begin with the general systems idea, then compare it with the current Trit boundary before you touch a tool. A prerequisite is a reasoning dependency, not a UI gate: a direct link to this page must still explain what is assumed.
...
[Continue to the next lesson](/learn/eecs/full-system-validation). The next page should make the dependency explicit and keep the same distinction between general concept, current Trit behavior, and planned work. When you reach the terminal lesson, write the closure report and list residual gaps instead of assuming that navigation itself proves completion.
